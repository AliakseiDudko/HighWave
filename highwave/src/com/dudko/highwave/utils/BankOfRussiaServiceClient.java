...
public class BankOfRussiaServiceClient {
	public static List<Entry<LocalDate, Double>> getExchangeRateHistory(Currency currency) {
		List<Entry<LocalDate, Double>> list = new ArrayList<Entry<LocalDate, Double>>();

		LocalDate endDate = MinskLocalDate.now();
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
				...
				LocalDate onDate = LocalDate.parse(onDateString, DateTimeFormat.forPattern("dd.MM.yyyy"));
				...
				list.add(new AbstractMap.SimpleEntry<LocalDate, Double>(onDate, currencyRate));
			}
		}
